defmodule Maps do
  ...
  koan "A map has keys and values" do
    assert Map.keys(@person) |> Enum.sort == [:age, :last_name, :name]
    assert Map.values(@person) |> Enum.sort == [27, "Jon", "Snow"]
  end
  ...
  koan "Can remove pairs by key" do
    ...
    assert Map.keys(without_age) |> Enum.sort == [:last_name, :name]
  end

  koan "Can merge maps" do
    assert Map.merge(%{name: "Jon"}, %{last_name: "Snow"}) == %{
     name: "Jon",
     last_name: "Snow"
   }
  end
  ...
  koan "You can also select sub-maps out of a larger map" do
    ...
    assert Map.take(initial, [:name, :last_name]) == %{
     name: "Jon",
     last_name: "Snow"
     }
  end
